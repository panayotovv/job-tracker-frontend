...
export default function Topbar({ onOpenAuth, search, setSearch, user, jobCount, newToday}) {
    ...
    <div className="relative flex items-center justify-between px-7 py-3.5 bg-[#0a0f1e] border-b border-indigo-500/10 backdrop-blur-md">
      ...
      <div className="flex items-center gap-2">
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-indigo-500/13 bg-white/[0.03]">
          ...
          <span className="text-[13px] font-semibold text-slate-200">{jobCount.toLocaleString()}</span>
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-indigo-500/13 bg-white/[0.03]">
          ...
          <span className="text-[13px] font-semibold text-slate-200">{newToday}</span>
      ...
      <div className="flex items-center gap-2.5">
        <div className="relative">
          ...
          <input
            ...
            className={`pl-8 pr-3 py-2 rounded-[10px] text-[13.5px] text-slate-300 placeholder:text-slate-600 bg-white/[0.04] border border-indigo-500/15 outline-none transition-all duration-300 ease-in-out caret-indigo-400 focus:border-indigo-500/50 focus:bg-indigo-500/5 focus:shadow-[0_0_0_3px_rgba(99,102,241,0.1)] ${focused ? "w-60" : "w-44"}`}
          />
        </div>

        <button className="relative w-[38px] h-[38px] rounded-[10px] border border-indigo-500/15 bg-white/[0.04] flex items-center justify-center text-slate-400 hover:text-indigo-300 hover:bg-indigo-500/12 hover:border-indigo-500/35 cursor-pointer">
        ...
        <div className="w-px h-6 bg-indigo-500/15" />
          ...
          {!user ? (
            <button
                onClick={onOpenAuth}
                className="flex items-center gap-1.5 text-xs px-4 py-2 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 hover:bg-indigo-500/20 hover:text-indigo-300 transition-all cursor-pointer"
                >
          ...
          <div className="relative cursor-pointer group">
            {user.image ? (
              ...
              <img
                src={user.image}
                alt="User avatar"
                className="w-9 h-9 rounded-full border-[1.5px] border-indigo-500/40 group-hover:border-indigo-500/80 transition-colors duration-200 object-cover block"
